...
| 4 | **Шипики 133s — параллелизация по веткам** | `spine_utils.calculate_spines_on_neuron` | **высоко** | ветки независимы; RAM-стена частично снята — см. ниже |
...
⚠️ **НЕРЕШЁНО до любых правок шипиков:** в прошлой попытке `br.spines` читался как **0** на
всех 28 ветках — надо найти, где спайны реально хранятся (`br.spines` / `br.spines_obj` /
property?), иначе output-preserving не проверить. `calculate_spines_on_neuron` зовётся из
`neuron.py:2646`.
...
1. **Попробовать параллелизацию шипиков (Путь A)** — форк с облегчённым нейроном. Замерить RSS
   per-worker и сравнить с последовательным временем. Предварительно разобраться, где хранится
   `br.spines` (читались как 0 в прошлой попытке).
2. **Decimator → open3d** (§5 #2) — output-changing, −12s, судить против Neuron-baseline.

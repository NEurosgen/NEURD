...
**Рычаги для большого (по убыванию, за Docker-точностью НЕ гонимся):**
1. ⭐ **Poisson 766s/17 вызовов — depth 11→9.** Poisson ≈ ×8 на каждый +1 глубины, 11→9 ≈ ~4× по
   этим 766s. Риск: меняет реконструкцию сомы → возможно иное число лимбов; нужна 75-мин валидация.
   Сначала стоит понять, **откуда 17 вызовов** (соме-экстракция в цикле), нет ли лишних.
2. **meshlab-сабпроцессы ~346s** — перенести Decimator/Interior/FillHoles в in-process pymeshlab
   (как уже сделали с Poisson). Убирает накладные xvfb/subprocess.
3. ❌ `soma_volume_ratio` real-Poisson (~250s) — рассматривали убрать, но это ~5% на фоне 766s
   самого Poisson; не стоит трогать (пользователь согласен).

### RAM на реальных H01
Малый (507К граней): пик ~1.65 ГБ (после удешевления сомы; было ~1.8). Большой `1830470325`
(3.27М граней): пик **~8.27 ГБ**. Децимация уже идёт внутри соме-экстракции (0.25×0.25) — НЕ рычаг.
...
> ⭐ **АКТУАЛЬНЫЕ приоритеты (2026-05-31) — см. §0 «Большой нейрон».** Профиль большого H01
> сместил картину: доминанта — **real Poisson depth=11 (766s)** + meshlab-сабпроцессы (346s),
> а НЕ шипики (заброшены) и НЕ скелетонизация. Топ-рычаги: **(1) Poisson depth 11→9** (~4× по
> 766s), **(2) meshlab Decimator/Interior/FillHoles → in-process pymeshlab** (−346s). Таблица
> ниже — история малого нейрона/fixture; для большого она нерелевантна.

...
### ⭐ Poisson depth-sweep (2026-05-31) — depth=9 это «пол» и рабочая точка
...
`NEURD_SOMA_OUTER_DECIM` / `NEURD_SOMA_INNER_DECIM` (0.25).

| Конфиг | Время | Лимбы | Poisson self / вызовов | RAM | Сома |
|---|---|---|---|---|---|
| depth=11 (Docker) | 4515s (75м) | 12 | 766s / 17 | 8.27 ГБ | ✅ |
| **depth=9** ⭐ | **1589s (26.5м)** | **5** | **167s / 6** | 7.8 ГБ | ✅ |
| depth=7 | 179s† | — | (умер) | 2.95 ГБ | ❌ 0 сом «No Somas» |
| depth=9 + decim0.15 + iters6 | 1397s† | — | 187s / **11** | 4.8 ГБ | ❌ «not just one mesh» |
| *Docker* | — | *4* | — | — | ✅ |

† умер на полпути, не досчитал.

**Выводы:**
- ⭐ **depth=9 — чистый выигрыш по ВСЕМ осям: −2.84× время, −4.6× Poisson, ниже RAM, и разбиение
  (5 лимбов) БЛИЖЕ к Docker (4), чем depth=11 (12).** Контринтуитивно: depth=11 переобучается на
  шумной EM-поверхности → фрагментирует; depth=9 сглаживает щели → мостит отростки (как Docker с его
  шагом сглаживания нормалей, которого у нас нет).
- **depth=7 = ниже порога детекции сомы** → 0 сом → краш. depth=9 — у «пола», но держится.
- 🔴 **Агрессивная децимация соме-меша КОНТРПРОДУКТИВНА (опровергнута замером).** Идея: грубее меш →
  ...
  `correspondence_1_to_1 "not just one mesh"` (огрубление < порога связности). **Poisson/сома-рычаг
  ИСЧЕРПАН на depth=9** — дальше ломается.

**Рычаги для большого ПОСЛЕ depth=9 (Poisson выжат, дальше НЕ сома):**
1. ⭐ **meshlab-сабпроцессы ~262s** (Decimator 109 + Interior 66 + FillHoles) — перенести в in-process
   pymeshlab (как Poisson). **Не трогает реконструкцию → низкий риск краша.** Теперь это рычаг №1.
2. `closest_distance_between_meshes` 151s — геометрия привязки floating-кусков (сложнее, output-changing).
3. ❌ `soma_volume_ratio` real-Poisson — ~5% на фоне, не трогаем.
4. ✅ **depth=9 — зафиксировать дефолтом** (через `NEURD_POISSON_DEPTH=9` в process_all_neurons или
   дефолт хука). Пользователь думает; пока не закоммичено как дефолт.

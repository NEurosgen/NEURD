...
### Шаг F — провести конвейер сегментации до конца (Фаза 2/3, ПРИОРИТЕТНО)

Стрип зависимостей по сути завершён; главный незакрытый риск — **работает ли сама
сегментация** после всех удалений и перехода на numpy≥2 / trimesh≥4. Конвейер ни
разу не прогонялся на новом стеке.

**Полный план и пошаговый цикл «прогон → фикс → прогон» — в [PIPELINE.md](PIPELINE.md)**
(раздел «Что делать дальше»). Кратко:
1. Прогнать `tests/integration/test_autoproof_pipeline.py` (xvfb+meshlab нужны).
2. Чинить первый реальный compat-баг (не каскадные `AttributeError`).
3. Повторять, пока все 9 стадий не пройдут.

Это **точечные багфиксы** numpy2/trimesh4, НЕ замена бинарников (MeshLab/CGAL —
отдельная оптимизация, кандидаты перечислены в PIPELINE.md, сейчас не трогаем).
Первый известный блокер: `soma_extraction_utils.py:1002` (trimesh≥4 `.split()` →
list, индексируется np-массивом). Прогресс: стадии 0–2 PASS, стадия 3 блокирована.
